...
export default function Home() {
  ...
  const parsedAmt = useMemo(
    () => isValidFormat ? parseFloat(amount) : NaN,
    [amount, isValidFormat]
  );
    ...
    <>
      ...
      >
        ...
        <section className="bg-white bg-opacity-90 p-6 rounded-2xl shadow-xl max-w-sm mx-auto mb-6 text-center">
          ...
          {isConnected && (
            <>
              {!onBase && <p className="text-red-600 mb-2">Please switch your wallet to Base network.</p>}
              {!isAllowed ? (
        ...
        <section className="bg-white bg-opacity-90 p-6 rounded-2xl shadow-xl max-w-sm mx-auto mb-6 text-center">
          ...
          <button
            onClick={() => redeemWrite?.()}
            disabled={!redeemWrite || isRedeeming || parsedRedeem <= 0}
