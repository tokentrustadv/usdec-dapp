...
export default function Home() {
  ...
  const fullAmount    = isValidAmount
    ? BigInt(Math.floor(parsedAmt * 1e6))
    : undefined;
  ...
  const { config: redeemConfig } = usePrepareContractWrite({
    ...
    abi: usdecAbi,
    functionName: 'redeem',
    args: redeemAmount ? [BigInt(Math.floor(parseFloat(redeemAmount)*1e6))] : undefined,
    enabled: isConnected && parseFloat(redeemAmount) > 0,
  });
  const {
    write: redeemWrite,
  ...
  const usdcBal    = useContractRead({
    address: BASE_USDC_ADDRESS, abi: erc20ABI,
    functionName: 'balanceOf', args: [address],
    enabled: isConnected, watch: true
  }).data ?? 0n;
  const usdecBal   = useContractRead({
    address: USDEC_ADDRESS, abi: usdecAbi,
    functionName: 'balanceOf', args: [address],
    enabled: isConnected, watch: true
  }).data ?? 0n;
  const unlocked   = useContractRead({
    address: USDEC_ADDRESS, abi: usdecAbi,
    functionName: 'unlockedBalance', args: [address],
    enabled: isConnected, watch: true
  }).data ?? 0n;
  ...
  // Reset approval if user changes amount
    ...
    <>
      ...
      <main className="min-h-screen p-4 bg-cover bg-center" style={{ backgroundImage: "url('/koru-bg-wide.png')" }}>
                ...
                : (
                  <>
                    ...
                    {parsedAmt < MIN_INPUT && amount !== '' && (
                      <p className="text-red-600 text-sm mb-2">Min {MIN_INPUT} USDC required</p>
                    )}
                    {isValidAmount && (
        ...
        <section className="bg-white bg-opacity-90 p-6 rounded-2xl shadow-xl max-w-sm mx-auto mb-6 text-center">
          ...
          <button
            onClick={() => redeemWrite?.()}
            disabled={!redeemWrite||isRedeeming||parseFloat(redeemAmount)<=0}
